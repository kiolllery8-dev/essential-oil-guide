...
(function(){
  ...
  byId('num-go').onclick=function(){
    ...
    // ── 四階高峰數 ─────────────────────────────────────────────
    // 元件用 reduceMaster（11 月＝11、數字和為 11／22 的年份保留原形），
    // 否則高峰數 22 在數學上永遠不可能出現（純化約時 9+9=18 是上限）。
    // 挑戰數／限制數仍用完整化約的 rm/rd/ry，兩套不共用。
    var pm=reduceMaster(m),pd2=reduceMaster(d),py=reduceMaster(y);
    // 第三、四階不化約 10／11／22：走過前兩階淬鍊的人，
    // 不會再回到第一、二階那種「從零開始學」的狀態，所以這些數字保留原形。
    function pinnacleOf(sum,stage){
      if(stage>=3&&(sum===10||sum===11||sum===22))return sum;
      return reduceNum(sum);
    }
    var p1=pinnacleOf(pm+pd2,1),p2=pinnacleOf(pd2+py,2);
    var p3=pinnacleOf(p1+p2,3),p4=pinnacleOf(pm+py,4);
  ...
  function render(life,bday,talent,zo,innate,counts,missing,active,flow,ty,combo,stages,curStage,age,lunarNote,flowInfo,zodiac,extra){
    ...
    h+='<div style="font-size:12px;color:#9A8AA8;margin-top:8px;">※ 高峰數＝月＋日、日＋年、前兩者相加、月＋年。月、日、年先各自縮減（11 月與數字和為 11／22 的年份保留原形），第三、四階的 10／11／22 不再縮減。第一階從出生到 '+(36-reduceNum(life))+' 歲（36 減主命數），之後每九年一段；年齡為粗估，實際以生日當天為準。</div></div>';
