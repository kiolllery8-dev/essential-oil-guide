...
PINNACLE_THEME = {
    1: dict(
        ...
        deeper='高峰數 1 只會在第一或第二階出現（有時兩階都是）。在第三或第四階，它會變成 10。'
               ...
               '取決於你的生命靈數藍圖所顯示的個性優勢。',
        modern='現代對應：這是最適合開始「以自己為名」的時期——創業、獨立接案、'
    ...
    3: dict(
        ...
        modern='現代對應：內容創作者、YouTuber、Podcast 主持、短影音、'
               '設計、文案、行銷企劃、講師、活動主持。'
               '這是自媒體與個人 IP 最容易起飛的時期——你的辨識度會自然變高。'
...
(function(){
  ...
  function render(life,bday,talent,zo,innate,counts,missing,active,flow,ty,combo,stages,curStage,age,lunarNote,flowInfo,zodiac,extra){
    ...
    h+='<p style="font-size:12.5px;color:#9A8AA8;margin:0 0 12px;">每一階都可以展開看完整說明。<b>第三、第四階若出現 10、11、22，會保留原本的數字</b>——走過前兩階的人不會再回到從零學的狀態。</p>';
    ...
    h+='<div style="font-size:12px;color:#9A8AA8;margin-top:8px;">※ 年、月、日各自先化約成個位數，再套：第一高峰數＝生月數＋生日數、第二＝生日數＋生年數、第三＝第一＋第二、第四＝生月數＋生年數。<b>第三、四階遇到 10、11、22 時保留原數</b>。年齡分界：第一階 0–'+extra.e1+' 歲（36 減命運數 '+extra.destiny+'），第二階起每九年一段；年齡為粗估，實際以生日當天為準。</div></div>';
    ...
    h+='<div style="font-size:12px;color:#9A8AA8;margin-top:6px;">※ 挑戰數＝|生月數−生日數|、|生年數−生日數|、兩者之差、|生年數−生月數|（各數先化約成個位數）。年齡分界與上方的高峰數相同——同一段人生，高峰數說你被推去發展什麼，挑戰數說你在那段時間會反覆卡在哪裡。第三階段的挑戰數影響最深，常被視為一生的主課題。</div></div>';
